...
# ----------------------------
def build_all_candidate_pairs_and_labels(
    resA_atoms,
    resB_atoms,
    idx_A,
    idx_B,
...
def stable_seed_from_case(case_name, base_seed):
    value = base_seed
...
def balance_train_pairs(pairs, labels, negative_ratio, seed):
    ...
    max_neg = min(len(neg_idx), len(pos_idx) * negative_ratio)
    sampled_neg_idx = rng.choice(neg_idx, size=max_neg, replace=False)

    selected_idx = np.concatenate([pos_idx, sampled_neg_idx])
    rng.shuffle(selected_idx)

    return pairs[selected_idx], labels[selected_idx]
...
def save_summary_md(path, summary, split_map, args):
    ...
    train_items = [item for item in summary if item["split"] == "train"]
    val_items = [item for item in summary if item["split"] == "val"]
    test_items = [item for item in summary if item["split"] == "test"]
    ...
    train_nodes, train_pos, train_neg, train_ratio = totals(train_items)
    val_nodes, val_pos, val_neg, val_ratio = totals(val_items)
    test_nodes, test_pos, test_neg, test_ratio = totals(test_items)

    total_ratio = total_saved_pos / total_saved_nodes if total_saved_nodes > 0 else 0.0

    with open(path, "w", encoding="utf-8") as f:
        f.write("# Train-Balanced / Natural-Evaluation Dataset Summary\n\n")

        f.write("This dataset balances only the training split while keeping validation and test splits natural.\n\n")

        f.write("## Configuration\n\n")
        f.write(f"- Cases CSV: `{args.cases}`\n")
        f.write(f"- Output directory: `{args.out_dir}`\n")
        f.write(f"- Train negative ratio: `{args.train_negative_ratio}`\n")
        ...
        f.write(f"| Total | {total_saved_nodes} | {total_saved_pos} | {total_saved_neg} | {total_ratio:.4f} |\n\n")

        f.write("## Per-Case Summary\n\n")
        f.write(
            "| Case | Split | Original Nodes | Original Pos | Original Neg | "
            "Saved Nodes | Saved Pos | Saved Neg | Saved Pos Ratio | Edges | Balanced? |\n"
        )
        f.write(
            "|------|-------|----------------|--------------|--------------|"
            "-------------|-----------|-----------|-----------------|-------|-----------|\n"
        ...
        for item in summary:
            f.write(
                ...
                f"| {item['edges']} "
                f"| {item['balance_applied']} |\n"
            )

        f.write("\n## Interpretation\n\n")
        f.write("- Training graphs are semi-balanced using all positives and sampled negatives.\n")
        f.write("- Validation and test graphs keep their natural class imbalance.\n")
        f.write("- This setup is more scientifically comparable to the original strict natural-test result.\n")
...
def build_case(row, split, args):
    ...
    all_pairs, all_labels = build_all_candidate_pairs_and_labels(
    ...
    balance_applied = split == "train"

    if balance_applied:
        case_seed = stable_seed_from_case(case_name, args.seed)
        pairs, labels = balance_train_pairs(
            pairs=all_pairs,
            labels=all_labels,
        ...
        pairs, labels = all_pairs, all_labels
    ...
    return {
        ...
        "balance_applied": str(balance_applied),
    }
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Build train-balanced / natural-val-test dataset."
    )
    ...
    parser.add_argument(
        "--out_dir",
        default=os.path.join("data", "processed_train_balanced_natural_test"),
    )

    parser.add_argument("--train_negative_ratio", type=int, default=3)
...
def main():
    ...
    print("\nTrain-balanced / natural-evaluation dataset build")
    print(f"Cases CSV: {args.cases}")
    print(f"Output dir: {args.out_dir}")
    print(f"Train negative ratio: {args.train_negative_ratio}")
    ...
    summary_csv = os.path.join(args.out_dir, "train_balanced_natural_eval_summary.csv")
    summary_md = os.path.join(args.out_dir, "train_balanced_natural_eval_summary.md")
