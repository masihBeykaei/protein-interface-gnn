...
def save_md(path, row, train_cases, val_cases, test_cases):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write("```text\n")
        f.write("Previous best GAT: F1 = 0.2924\n")
        f.write("TransformerConv best single run: F1 = 0.4134\n")
        f.write("TransformerConv mean over 4 seeds: F1 ≈ 0.3310\n")
        f.write("```\n")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train GCN on the combined current + BM5 ESM-2 PCA16 dataset."
    )
    ...
    parser.add_argument(
        "--output_csv",
        default=os.path.join("experiments", "combined_current_bm5_esm2_pca16_gcn_results.csv"),
    )

    parser.add_argument(
        "--output_md",
        default=os.path.join("experiments", "combined_current_bm5_esm2_pca16_gcn_results.md"),
    )
...
def main():
    ...
    for epoch in range(args.max_epochs + 1):
        ...
        print(
            f"GCN | Epoch {epoch} | Loss={avg_loss:.4f} "
        ...
        if epochs_without_improvement >= args.patience:
            print(f"GCN early stopping at epoch {epoch}")
            break
