...
class TransformerConvNet(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels=16, heads=4, dropout=0.3, beta=True):
        super().__init__()
...
def save_md(path, row, train_cases, val_cases, test_cases):
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write("# TransformerConv Experiment Result\n\n")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train TransformerConv on the combined current + BM5 ESM-2 PCA16 dataset."
    )
    ...
    parser.add_argument(
        "--output_csv",
        default=os.path.join("experiments", "combined_current_bm5_esm2_pca16_transformerconv_results.csv"),
    )

    parser.add_argument(
        "--output_md",
        default=os.path.join("experiments", "combined_current_bm5_esm2_pca16_transformerconv_results.md"),
    )
...
def main():
    ...
    for epoch in range(args.max_epochs + 1):
        ...
        print(
            f"TransformerConv | Epoch {epoch} | Loss={avg_loss:.4f} "
        ...
        if epochs_without_improvement >= args.patience:
            print(f"TransformerConv early stopping at epoch {epoch}")
            break
    ...
    print("\nTransformerConv experiment completed successfully.")
