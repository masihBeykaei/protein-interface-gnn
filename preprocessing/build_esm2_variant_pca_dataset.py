...
def build_raw_pair_features(pairs, emb1, emb2, mode):
    a_idx = pairs[:, 0].astype(np.int64)
    b_idx = pairs[:, 1].astype(np.int64)

    if len(a_idx) > 0 and a_idx.max() >= emb1.shape[0]:
        raise IndexError(
            f"Partner 1 pair index out of range: max={a_idx.max()}, embeddings={emb1.shape[0]}"
        )

    if len(b_idx) > 0 and b_idx.max() >= emb2.shape[0]:
        raise IndexError(
            f"Partner 2 pair index out of range: max={b_idx.max()}, embeddings={emb2.shape[0]}"
        )
    ...
    if mode == "absdiff":
        ...
        raw = product
    elif mode == "absdiff_product":
        raw = np.concatenate([abs_diff, product], axis=1)
    elif mode == "concat_ab_absdiff":
        raw = np.concatenate([emb_a, emb_b, abs_diff], axis=1)
    else:
...
def build_train_matrix(source_processed_dir, embedding_dir, train_cases, mode):
    matrices = []
    ...
    for case_name in train_cases:
        ...
        raw_pair = build_raw_pair_features(pairs, emb1, emb2, mode)
        matrices.append(raw_pair)

        print(f"Loaded train raw pair matrix for {case_name}: {raw_pair.shape}")
...
def save_summary_md(path, rows, args, final_dim, raw_dim, actual_components, explained_var):
    ...
    mode_text = {
        "absdiff": "abs(ESM_A - ESM_B)",
        "product": "ESM_A * ESM_B",
        "absdiff_product": "abs(ESM_A - ESM_B), ESM_A * ESM_B",
        "concat_ab_absdiff": "ESM_A, ESM_B, abs(ESM_A - ESM_B)",
    }[args.mode]

    with open(path, "w", encoding="utf-8") as f:
        f.write("# ESM-2 Variant PCA Pair-Feature Dataset Summary\n\n")
        ...
        f.write(f"- Raw pair feature: `{mode_text}`\n")
        f.write(f"- Basic feature count: `{args.basic_feature_count}`\n")
        f.write(f"- Raw pair dimension: `{raw_dim}`\n")
        ...
        f.write("```text\n")
        f.write(f"[basic_3_features, PCA({mode_text})]\n")
        f.write("```\n\n")

        f.write("Standardization and PCA are fitted only on training pairs.\n\n")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Build variant ESM-2 PCA pair features for correspondence graphs."
    )

    parser.add_argument(
        "--source_processed_dir",
        default=os.path.join("data", "processed_combined_current_bm5_train_balanced_natural_test"),
    )

    parser.add_argument(
        "--embedding_dir",
        default=os.path.join("data", "esm2_embeddings"),
    )
...
def main():
    ...
    print("\nBuilding variant ESM-2 PCA correspondence features")
    ...
    raw_dim = int(train_matrix.shape[1])
    actual_components = min(args.pca_components, raw_dim, train_matrix.shape[0])

    print(f"Train raw pair matrix shape: {train_matrix.shape}")
    print(f"Raw pair feature dimension: {raw_dim}")
    ...
    with open(os.path.join(args.out_dir, "esm2_variant_pair_scaler.pkl"), "wb") as f:
        pickle.dump(scaler, f)

    with open(os.path.join(args.out_dir, "esm2_variant_pair_pca.pkl"), "wb") as f:
        pickle.dump(pca, f)

    shutil.copyfile(
        os.path.join(args.source_processed_dir, "split_cases.json"),
        os.path.join(args.out_dir, "split_cases.json"),
    )
    ...
    for case_name in all_cases:
        ...
        basic_features, labels, pairs, edge_index = load_case_arrays(
            args.source_processed_dir,
            case_name,
        )

        emb1, emb2 = load_embeddings(args.embedding_dir, case_name)
        raw_pair = build_raw_pair_features(pairs, emb1, emb2, args.mode)

        raw_scaled = scaler.transform(raw_pair)
        pca_features = pca.transform(raw_scaled).astype(np.float32)

        basic = basic_features[:, :args.basic_feature_count].astype(np.float32)
        final_features = np.concatenate([basic, pca_features], axis=1).astype(np.float32)
        ...
        rows.append({
            ...
            "raw_pair_dim": raw_dim,
            "pca_components": actual_components,
            "mode": args.mode,
        })

        print(
            f"{case_name}: split={split}, nodes={len(labels)}, "
            f"positive={pos}, feature_dim={final_dim}"
        )
    ...
    with open(os.path.join(args.out_dir, "esm2_variant_pca_feature_info.json"), "w", encoding="utf-8") as f:
        json.dump(info, f, indent=2)

    summary_csv = os.path.join(args.out_dir, "esm2_variant_pca_dataset_summary.csv")
    summary_md = os.path.join(args.out_dir, "esm2_variant_pca_dataset_summary.md")
    ...
    print("Variant ESM-2 PCA feature dataset completed successfully.")
