...
def print_graph_summary(title, graphs):
    ...
    for case_name, graph in graphs.items():
        ...
        print(
            f"{case_name}: nodes={graph.num_nodes}, "
            f"edges={graph.edge_index.shape[1]}, "
            f"positive={pos}, negative={neg}, "
            f"feature_dim={graph.x.shape[1]}"
        )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train GIN on the combined current + BM5 ESM-2 PCA16 dataset."
    )
    ...
    parser.add_argument(
        "--output_csv",
        default=os.path.join("experiments", "combined_current_bm5_esm2_pca16_gin_results.csv"),
    )

    parser.add_argument(
        "--output_md",
        default=os.path.join("experiments", "combined_current_bm5_esm2_pca16_gin_results.md"),
    )
...
def main():
    ...
    for epoch in range(args.max_epochs + 1):
        ...
        print(
            f"GIN | Epoch {epoch} | Loss={avg_loss:.4f} "
        ...
        if epochs_without_improvement >= args.patience:
            print(f"GIN early stopping at epoch {epoch}")
            break
    ...
    print("\nGIN experiment completed successfully.")
